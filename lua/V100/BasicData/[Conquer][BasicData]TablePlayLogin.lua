...
function System_PlayLogin()
	...
	if SpecialServer_ChkNoGiftServer()then
		for _,func in ipairs(tSystem_PlayLogin_Func_NoGift) do
			if func ~= nil and type(func) == "function" then
				func(ndate,nweek,nHour,nMinute)
			end
		end
...
tSystem_PlayLoginFirst_Func_NoGift = {}
...
function System_PlayLoginFirst()
	...
	-- 判断是否是无赠品服务器
	if SpecialServer_ChkNoGiftServer()then
		for _,func in ipairs(tSystem_PlayLoginFirst_Func_NoGift) do
			if func ~= nil and type(func) == "function" then
				func(ndate,nweek,nHour,nMinute)
			end
		end
