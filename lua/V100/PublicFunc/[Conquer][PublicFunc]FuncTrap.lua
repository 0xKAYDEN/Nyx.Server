...
function Trap_CreateMapTrap(nType,nLook,nOwnerId,nMapId,nPosX,nPosY,nPosCX,nPosCY)	
	...
	if type(nType) ~= "number" or  nType%1 ~= 0 or nType <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap [nType,nMapId]:[".. nType ..",".. nMapId .."]中 nType 只能传大于0的整数")
		return
	end
	
	if type(nLook) ~= "number" or nLook%1 ~= 0 or nLook < 0 then
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap [nType,nMapId]:[".. nType ..",".. nMapId .."]中 nLook 只能传大于等于0的整数")
		return
	end
	
	if type(nOwnerId) ~= "number" or nOwnerId%1 ~= 0 or nOwnerId < 0 then
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap [nType,nMapId]:[".. nType ..",".. nMapId .."]中 nOwnerId 只能传大于等于0的整数")
		return
	end
	
	if type(nMapId) ~= "number" or nMapId%1 ~= 0 or nMapId <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap [nType,nMapId]:[".. nType ..",".. nMapId .."]中 nMapId 只能传大于0的整数")
		return
	end
	
	if type(nPosX) ~= "number" or nPosX%1 ~= 0 or nPosX <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap [nType,nMapId]:[".. nType ..",".. nMapId .."]中 nPosX 只能传大于等于0的整数")
		return
	end
	
	if type(nPosY) ~= "number" or nPosY%1 ~= 0 or nPosY <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap [nType,nMapId]:[".. nType ..",".. nMapId .."]中 nPosY 只能传大于等于0的整数")
		return
	end
	
	if type(nPosCX) ~= "number" or nPosCX%1 ~= 0 or nPosCX < 0 then
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap [nType,nMapId]:[".. nType ..",".. nMapId .."]中 nPosCX 只能传大于等于0的整数")
		return
	end
	
	if type(nPosCY) ~= "number" or nPosCY%1 ~= 0 or nPosCY < 0 then
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap [nType,nMapId]:[".. nType ..",".. nMapId .."]中 nPosCY 只能传大于等于0的整数")
		return
	end
	
	if CreateMapTrap(nType,nLook,nOwnerId,nMapId,nPosX,nPosY,0,nPosCX,nPosCY) then
		MoveTrap_SetCount(nType,nMapId,2)
		return true
	else
		Sys_SaveAbnormalLog("函数 Trap_CreateMapTrap 创建陷阱TYPE为：" .. nType .."失败。")
		return false
	end
end
...
function Trap_EraseMapTrap(nTrapId)
	...
	elseif type(nTrapId) ~= "number" or nTrapId%1 ~= 0 or nTrapId < 0 then
		Sys_SaveAbnormalLog("函数 Trap_EraseMapTrap 中 [nTrapId]:[".. nTrapId .."] 只能是大于等于0的整数")
		return
	end
...
function Trap_DelMapTrap(nMapId,nType)	
	if type(nMapId) ~= "number" or nMapId%1 ~= 0 or nMapId <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_DelMapTrap [nMapId,nType]:[".. nMapId ..",".. nType .."]中 nMapId 只能传大于0的整数")
		return
	end
	
	if type(nType) ~= "number" or nType%1 ~= 0 or nType <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_DelMapTrap [nMapId,nType]:[".. nMapId ..",".. nType .."]中 nType 只能传大于0的整数")
		return
	end
	
	if DelMapTrap(nMapId,nType) then
		MoveTrap_SetCount(nType,nMapId,1)
		return true
	else
		return false
	end
...
function Trap_ChangeTrapType(nTrapId,nData)
	if type(nTrapId) ~= "number" or nTrapId%1 ~= 0 or nTrapId <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_ChangeTrapType 中 [nTrapId]:[".. nTrapId .."] 只能传大于0的整数")
		return
	end
	
	if type(nData) ~= "number" or nData%1 ~= 0 or nData%1 ~= 0 or nData <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_ChangeTrapType [nTrapId]:[".. nTrapId .."]中 nData 只能传大于0的整数")
		return
	end
...
function Trap_ChangeTrapLook(nTrapId,nData)
	if type(nTrapId) ~= "number" or nTrapId%1 ~= 0 or nTrapId <= 0 then
		Sys_SaveAbnormalLog("函数 Trap_ChangeTrapLook 中 [nTrapId]:[".. nTrapId .."] 只能传大于0的整数")
		return
	end
	
	if type(nData) ~= "number" or nData%1 ~= 0 or nData < 0 then
		Sys_SaveAbnormalLog("函数 Trap_ChangeTrapLook [nTrapId]:[".. nTrapId .."]中 nData 只能传大于等于0的整数")
		return
	end
