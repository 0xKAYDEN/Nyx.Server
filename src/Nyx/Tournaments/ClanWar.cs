...
namespace Nyx.Server.Game
{
    ...
    {
        public readonly ILogger log = Log.ForContext<ClanWar>();
        ...
        public static GameTime ScoreSendStamp, LastWin;
        ...
        public static void Initiate()
        {
            ...
            if (Map.Npcs.TryGetValue(812, out var pole))
                Pole = (SobNpcSpawn)pole;
            else
                Log.Warning("ClanWar.Initiate: clan-war pole NPC (UID 812) not found on map 1509 — pole disabled");
        }
        public static void Start()
        {
            ...
            {
                client.Send(new Message("Clan war has began!", System.Drawing.Color.Red, Message.Center));
            }
            FirstRound = true;
        ...
        public static void FinishRound()
        {
            ...
            if (PoleKeeper != null)
            {
                ...
                foreach (var client in Kernel.GamePool.Values)
                {
                    client.Send(new Message("The Clan, " + PoleKeeper.Name + ", owned by " + PoleKeeper.LeaderName + " has won this Clan war round!", System.Drawing.Color.Red, Message.Center));
                }
        ...
        public static void End()
        {
            ...
            {
                foreach (var client in Kernel.GamePool.Values)
                client.Send(new Message("The clan, " + PoleKeeper.Name + ", owned by " + PoleKeeper.LeaderName + " has won this Clan war!---Clan war has ended!", System.Drawing.Color.White, Message.Center));
            ...
            {
                foreach (var client in Kernel.GamePool.Values)
                client.Send(new Message("Clan war has ended and there was no winner!", System.Drawing.Color.Red, Message.Center));
