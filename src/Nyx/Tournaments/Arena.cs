...
namespace Nyx.Server.Game
{
    public class Arena
    {
        ...
        public static ConcurrentDictionary<uint, ChampionPoints> ChampionArenaStatistics = new ConcurrentDictionary<uint, ChampionPoints>();
        ...
        private static void SaveArenaStats()
        {
            ...
            var array2 = ChampionArenaStatistics.Values.ToArray();
            foreach (ArenaStatistic stats in array)
                if (stats != null)
                    Database.ArenaTable.SaveArenaStatistics(stats, null);
            foreach (ChampionPoints stats in array2)
                if (stats != null)
                    Database.ArenaTable.SaveArenaStatistics(null, stats);
        }
        public static class QualifierList
        {
            ...
            public static AtomicCounter GroupCounter = new Nyx.Server.Utilities.AtomicCounter();
            ...
            public static byte[] BuildPacket(ushort page)
            {
                ...
                for (int count = page; count < page + 6; count++)
                {
                    ...
                    byte[] array = Program.Encoding.GetBytes(entry.Player1.ArenaStatistic.Name);
                    for (int i = 0; i < 16; i++)
                    {
                    ...
                    byte[] array2 = Program.Encoding.GetBytes(entry.Player2.ArenaStatistic.Name);
                    for (int i = 0; i < 16; i++)
                    {
            ...
            public class QualifierGroup
            {
                ...
                public byte[] BuildWatcherList(List<Client.GameClient> list, ushort id, uint Fighter = 0)
                {
                    var lenght = 58;
                    if (list.Count > 0)
                        lenght = 130;
                    byte[] buf = new byte[lenght + 8];

                    Network.SafeWriter.Write((ushort)lenght, 0, buf);

                    Network.SafeWriter.Write((ushort)2211, 2, buf);

                    Network.SafeWriter.Write((ushort)id, 4, buf);

                    Network.SafeWriter.Write((uint)0, 6, buf);

                    Network.SafeWriter.Write((uint)Fighter, 10, buf);

                    Network.SafeWriter.Write((uint)list.Count, 14, buf);

                    Network.SafeWriter.Write((uint)Player1Cheers, 18, buf);

                    Network.SafeWriter.Write((uint)Player2Cheers, 22, buf);
                    var offset = 26;
                    foreach (Client.GameClient client in list)
                    {
                        Network.SafeWriter.Write((uint)client.Entity.Mesh, offset, buf);
                        offset += 4;
                        Network.SafeWriter.Write(client.ArenaStatistic.Name, offset, buf);
                        offset += 16;
                        Network.SafeWriter.Write((uint)client.Entity.UID, offset, buf);
                        offset += 4;
                        Network.SafeWriter.Write((uint)client.ArenaStatistic.Level, offset, buf);
                        offset += 4;
                        Network.SafeWriter.Write((uint)client.ArenaStatistic.Class, offset, buf);
                        offset += 4;
                        Network.SafeWriter.Write((uint)client.ArenaStatistic.Rank, offset, buf);
                        offset += 4;
                    }
                    //MemoryStream strm = new MemoryStream();
                    //BinaryWriter wtr = new BinaryWriter(strm);
                ...
                public void BeginWatching(Client.GameClient client, uint Fighter)
                {
                    if (!client.Entity.Dead && client.WatchingGroup == null && client.QualifierGroup == null)
                    {
                        ...
                        client.Send(BuildWatcherList(new List<GameClient>(), 0, Fighter));
                        client.Send(this.match.BuildPacket(0));

                        Watchers.Add(client);
                        byte[] packet = BuildWatcherList(Watchers, 2, Fighter);
                ...
                public GameTime CreateTime;
                public GameTime DoneStamp;
                ...
                public void Import()
                {
                    ...
                    if (Player1.Map.ID == Player2.Map.ID)
                    {
                        ...
                        Player1.Entity.PKMode = Nyx.Server.Game.Enums.PKMode.PK;
                        ...
                        Player2.Entity.PKMode = Nyx.Server.Game.Enums.PKMode.PK;
        ...
        public class QualifyEngine
        {
            ...
            public static void DoQuit(Client.GameClient client)
            {
                ...
                if (client.QualifierGroup != null)
                {
                    ...
                    client.QualifierGroup.CreateTime = GameTime.Now.AddSeconds(-100);
                }
                else
                {
                    ArenaSignup sign = new ArenaSignup();
        ...
        public class ArenaSignup
        {
            ...
            public byte[] BuildPacket()
            {
                byte[] buf = new byte[56 + 8];
                if (Stats == null)
                {
                    Stats = new ArenaStatistic(true);
                    Stats.Name = "";
                }
                Network.SafeWriter.Write((ushort)56, 0, buf);
                Network.SafeWriter.Write((ushort)Type, 2, buf);
                Network.SafeWriter.Write((uint)DialogID, 4, buf);
                Network.SafeWriter.Write((uint)OptionID, 8, buf);
                Network.SafeWriter.Write((uint)Stats.EntityID, 12, buf);
                Network.SafeWriter.Write(Stats.Name, 20, buf);
                Network.SafeWriter.Write((uint)Stats.Class, 40, buf);
                Network.SafeWriter.Write((uint)Stats.Rank, 44, buf);
                Network.SafeWriter.Write((uint)Stats.ArenaPoints, 48, buf);
                Network.SafeWriter.Write((uint)Stats.Level, 52, buf);
                return buf;
            }
        }
        ...
        public class GroupMatch
        {
            public byte[] BuildPacket(byte valu = 1)
            {
                ...
                Network.SafeWriter.Write(60, 0, buff);
                Network.SafeWriter.Write(Type, 2, buff);
                Network.SafeWriter.Write((uint)Group.Player1.ArenaStatistic.EntityID, 4, buff);
                Network.SafeWriter.Write(Group.Player1.ArenaStatistic.Name, 12, buff);
                Network.SafeWriter.Write((uint)Group.Player1Damage, 28, buff);
                Network.SafeWriter.Write((uint)Group.Player2.ArenaStatistic.EntityID, 32, buff);
                Network.SafeWriter.Write(Group.Player2.ArenaStatistic.Name, 40, buff);
                Network.SafeWriter.Write((uint)Group.Player2Damage, 56, buff);
                return buff;
            }
        ...
        public static void Win(Client.GameClient winner, Client.GameClient loser)
        {
            if (winner.ArenaStatistic.PlayWith != null && loser.ArenaStatistic.PlayWith != null)
            {
                ...
                {
                    winner.CurrentHonor += 500;
                    winner.HistoryHonor += 500;
                    winner.Inventory.Add(723912, 0, 1);
                    winner.Send(new Message("You won a fight in the Qualifier, and received an Arena Pack!", Color.Red, Message.System));
                }
                if (loser.ArenaStatistic.TodayBattles == 20)
                {
                    loser.CurrentHonor += 500;
                    loser.HistoryHonor += 500;
                    loser.Inventory.Add(723912, 0, 1);
                    loser.Send(new Message("You won a fight in the Qualifier, and received an Arena Pack!", Color.Red, Message.System));
                }
                ...
                {
                    winner.ChampionPoints += 250;

                }
                if (loser.ArenaStatistic.TodayBattles == 1)
                {
                    loser.ChampionPoints += 100;

                }
                if (winner.ArenaStatistic.TodayBattles == 1)
                {
                    winner.ChampionPoints += 100;

                }
                if (loser.ArenaStatistic.TodayBattles == 3)
                {
                    loser.ChampionPoints += 150;

                }
                if (winner.ArenaStatistic.TodayBattles == 3)
                {
                    winner.ChampionPoints += 150;

                }
                if (loser.ArenaStatistic.TodayBattles == 5)
                {
                    loser.ChampionPoints += 200;
                }
                if (winner.ArenaStatistic.TodayBattles == 5)
                {
                    winner.ChampionPoints += 200;
                }
                if (loser.ArenaStatistic.TodayBattles == 7)
                {
                    loser.ChampionPoints += 200;
                }
                if (winner.ArenaStatistic.TodayBattles == 7)
                {
                    winner.ChampionPoints += 200;
                }
                #endregion
                ...
                builder.Append(".");
                if (loser.ArenaStatistic.Rank <= 500 && loser.ArenaStatistic.Rank != 0)
                {
                    if (loser.ArenaStatistic.Rank <= 10)
                        loser.Send(new Network.GamePackets.Message("You are now ranked No. " + loser.ArenaStatistic.Rank + " in the Arena! You are in the top 10!", Color.Red, Network.GamePackets.Message.System));
                    else if (loser.ArenaStatistic.Rank <= 25)
                        loser.Send(new Network.GamePackets.Message("You are now ranked No. " + loser.ArenaStatistic.Rank + " in the Arena! You are in the top 25!", Color.Red, Network.GamePackets.Message.System));
                    else if (loser.ArenaStatistic.Rank <= 50)
                        loser.Send(new Network.GamePackets.Message("You are now ranked No. " + loser.ArenaStatistic.Rank + " in the Arena! You are in the top 50!", Color.Red, Network.GamePackets.Message.System));
                    else if (loser.ArenaStatistic.Rank <= 100)
                        loser.Send(new Network.GamePackets.Message("You are now ranked No. " + loser.ArenaStatistic.Rank + " in the Arena! You are in the top 100!", Color.Red, Network.GamePackets.Message.System));
                    else if (loser.ArenaStatistic.Rank <= 250)
                        loser.Send(new Network.GamePackets.Message("You are now ranked No. " + loser.ArenaStatistic.Rank + " in the Arena! You are in the top 250!", Color.Red, Network.GamePackets.Message.System));
                    else if (loser.ArenaStatistic.Rank <= 500)
                        loser.Send(new Network.GamePackets.Message("You are now ranked No. " + loser.ArenaStatistic.Rank + " in the Arena! You are in the top 500!", Color.Red, Network.GamePackets.Message.System));
                }
                if (winner.ArenaStatistic.Rank <= 500 && winner.ArenaStatistic.Rank != 0)
                {
                    if (winner.ArenaStatistic.Rank <= 10)
                        winner.Send(new Network.GamePackets.Message("You are now ranked No. " + winner.ArenaStatistic.Rank + " in the Arena! You are in the top 10!", Color.Red, Network.GamePackets.Message.System));
                    else if (winner.ArenaStatistic.Rank <= 25)
                        winner.Send(new Network.GamePackets.Message("You are now ranked No. " + winner.ArenaStatistic.Rank + " in the Arena! You are in the top 25!", Color.Red, Network.GamePackets.Message.System));
                    else if (winner.ArenaStatistic.Rank <= 50)
                        winner.Send(new Network.GamePackets.Message("You are now ranked No. " + winner.ArenaStatistic.Rank + " in the Arena! You are in the top 50!", Color.Red, Network.GamePackets.Message.System));
                    else if (winner.ArenaStatistic.Rank <= 100)
                        winner.Send(new Network.GamePackets.Message("You are now ranked No. " + winner.ArenaStatistic.Rank + " in the Arena! You are in the top 100!", Color.Red, Network.GamePackets.Message.System));
                    else if (winner.ArenaStatistic.Rank <= 250)
                        winner.Send(new Network.GamePackets.Message("You are now ranked No. " + winner.ArenaStatistic.Rank + " in the Arena! You are in the top 250!", Color.Red, Network.GamePackets.Message.System));
                    else if (winner.ArenaStatistic.Rank <= 500)
                        winner.Send(new Network.GamePackets.Message("You are now ranked No. " + winner.ArenaStatistic.Rank + " in the Arena! You are in the top 500!", Color.Red, Network.GamePackets.Message.System));
                }
                foreach (var client in Kernel.GamePool.Values)
                    client.Send(new Network.GamePackets.Message(builder.ToString(), Color.Red, Network.GamePackets.Message.ArenaQualifier));
                winner.QualifierGroup = null;
                loser.QualifierGroup = null;
                    ...
                    for (; j != jEnd; j += jPlus)
                    {
                        ...
                        if (CanJoin(Challanged, Now) && Challanged.ArenaState == FindMatch)
                        {
                            if (Challanger.Entity.UID != Challanged.Entity.UID)
                            {
                                ...
                                Challanged.ArenaStatistic.AcceptBoxShow = Challanger.ArenaStatistic.AcceptBoxShow = GameTime.Now;
            ...
            foreach (var Challanger in WaitingPlayerList.Values)
            {
                if (Challanger != null)
                {
                    ...
                    if (Challanger.ArenaStatistic.PlayWith != null)
                    {
                        ...
                        {
                            if (GameTime.Now > Challanger.ArenaStatistic.AcceptBoxShow.AddSeconds(60))
                            {
        ...
        public static void CheckGroups()
        {
            if (QualifierList.Groups.Count > 0)
            {
                ...
                {
                    if (GameTime.Now > group.CreateTime.AddSeconds(5))
                    {
                        if (!group.Done)
                        {
                            if (GameTime.Now > group.CreateTime.AddMinutes(3))
                            {
                                group.End();
                            }
                        }
                        ...
                        {
                            if (GameTime.Now > group.DoneStamp.AddSeconds(4))
                            {
                                group.DoneStamp = GameTime.Now.AddDays(1);
                                group.Export();
                    ...
                    foreach (ArenaStatistic stat in array)
                    {
                        try
                        {
                            if (Kernel.GamePool.ContainsKey(stat.EntityID))
                            {
                                ...
                                Database.ArenaTable.Reset(client, stat, null);
                                client.Send(stat);
                            }
                            else
                                Database.ArenaTable.Reset(null, stat, null);
                        }
                        catch { }
                    }
                    var array2 = ChampionArenaStatistics.Values.ToArray();
                    foreach (ChampionPoints stat in array2)
                    {
                        try
                        {
                            if (Kernel.GamePool.ContainsKey(stat.EntityID))
                            {
                                Client.GameClient client = Kernel.GamePool[stat.EntityID];
                                Database.ArenaTable.Reset(client, null, stat);
                                client.Send(stat);
                            }
                            else
                                Database.ArenaTable.Reset(null, null, stat);
                        }
                        catch { }
                    }
                    try
        ...
        private static bool CanJoin(Client.GameClient client, GameTime now)
        {
            if (client != null)
            {
