...
namespace Nyx.Server.Game
{
    ...
    {
        public static int EventTime = 20;
        public class States
        {
            public const byte
                GUI_Top8Ranking = 0,
        ...
        public ConcurrentDictionary<uint, GameClient> Players;
        ...
        public ElitePK(int group)
        {
            if (!Alive)
            {
                Players = new ConcurrentDictionary<uint, GameClient>();
                ...
                LoadTop8(string.Format("Elite{0}", GroupID));

                var waitMapId = (ushort)ElitePKTournament.WaitingAreaID;
                if (!Kernel.Maps.TryGetValue(waitMapId, out var baseMap) || baseMap == null)
                {
                    Console.WriteLine(
                        $"[ElitePK] Waiting map {waitMapId} is not loaded (run DB + DMaps.Load / check GameMap.dat). Group {group} disabled.");
                    return;
                }
        ...
        private void SaveTop8(string epk)
        {
            try
            {
                ...
                string SQL = "UPDATE `josephvariable` SET data=@data where ID = '" + epk + "' ;";
        ...
        private void LoadTop8(string epk)
        {
            try
            {
                ...
                {
                    cmd.Select("josephvariable").Where("ID", epk);
                    using (MySqlReader rdr = new MySqlReader(cmd))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                command.Insert("josephvariable").Insert("ID", epk);
                                command.Execute();
                            }
                        }
                    }
        ...
        public class FighterStats
        {
            ...
            public override string ToString()
            {
                ...
                build.Append(UID + "#" + Name + "#" + Mesh + "#" + RceiveReward + "");
            ...
            public void Load(string Line)
            {
                ...
                Name = data[1];
                Mesh = uint.Parse(data[2]);
                RceiveReward = byte.Parse(data[3]);
            }
        }
        public class Match
        {
            ...
            public uint TimeLeft
            {
                get
                {
                    int val = (int)((ImportTime.AddMinutes(3).Millisecond - DateTime.Now.Millisecond) / 1000);
                    if (val < 0) val = 0;
                    return (uint)val;
                }
            }
            public ConcurrentDictionary<uint, GameClient> Watchers;
            ...
            public Match(params GameClient[] players)
            {
                Players = players;
                Watchers = new ConcurrentDictionary<uint, GameClient>();
            ...
            public void Import(StatusFlag flag = StatusFlag.OK)
            {
                ...
                if (Players.Length > 1)
                {
                    GameClient player1 = null, player2 = null;
                    for (int i = 0; i < Players.Length; i++)
                    {
            ...
            private void importPlayer(GameClient player, GameClient opponent)
            {
                if (Players.Length > 1)
                {
            ...
            public void End(GameClient client)
            {
                if (Players.Length == 1)
                {
            ...
            public GameClient targetOf(GameClient client)
            {
                for (int i = 0; i < Players.Length; i++)
                    if (Players[i] != null)
            ...
            public void Send(GameClient client, int gui_state)
            {
                ElitePKBrackets brackets = new ElitePKBrackets(true, 1);
                brackets.Group = (ushort)GroupID;
            ...
            public void BeginWatch(GameClient client)
            {
            ...
            public void LeaveWatch(GameClient client)
            {
            ...
            public void Cheer(GameClient client, uint fighter)
            {
            ...
            public GameClient Return(Func<FighterStats, bool> fn)
            {
        ...
        public void SignUp(GameClient client)
        {
            ...
            client.MessageBox("You are in the waiting area for the Elite PK Tournament.");
            ...
            try
            {
                ...
                if (State == States.GUI_ReconstructTop)
                {
                    if (Now64.AddMinutes(1) > ConstructTop8)
                    {
                        ...
                        {
                            client.Send(brackets);
                            client.Send(new ElitePKBrackets(true) { Group = (ushort)GroupID, Type = 6, OnGoing = false });
                        }
                ...
                if ((Now64.Hour == (EventTime + 1)) && Now64.Minute >= (3 - GroupID) * 2)//done okgirab b2a
                {
                    #region GetState
                    ...
                    if (State == States.GUI_Knockout)
                    {
                        if (pState == States.T_Organize)
                        {
                            if (Players.Count < 8)
                            {
                                ...
                                if (GroupID == ElitePKTournament.EPK_Lvl130Plus)
                                {
                                    int count = 8 - Players.Count;
                                    if (Players.Count == 1)
                                    {
                                        foreach (var player in Players.Values)
                                        {
                                            player.Entity.Teleport(1002, 300, 278);
                                            player.MessageBox("Congratulations! You are the Champion of Elite Pk!!");
                                            player.Entity.AddTopStatus(
                                                            (ulong)(((int)TitlePacket.Titles.ElitePKChamption_High)),
                                                            DateTime.Now.AddDays(7));
                                            player.Inventory.Add(720717, 0, 1);
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            player.Entity.Teleport(1002, 300, 278);
                                            player.MessageBox("ElitePk Need 8 Players To Start");
                                        }
                                    }
                                 bool done = true;
                            ...
                            else
                            {
                                ...
                                {
                                    client.Send(brackets);
                                    client.Send(new ElitePKBrackets(true) { Group = (ushort)GroupID, Type = 6, OnGoing = true });
                                }
                                }
                        }
                    ...
                    if (State == States.GUI_Top8Qualifier)
                    {
                        if (pState == States.T_Organize)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (var client in Kernel.GamePool.Values)
                                {
                                    client.Send(CreateBrackets(MatchArray, 0, ElitePKBrackets.UpdateList));
                                }
                        ...
                        else if (pState == States.T_Wait)
                        {
                            if (TimeLeft == 0)
                            {
                                ...
                                if (doneMatches == ExtendedMatchArray.Length)
                                {
                                    ...
                                    foreach (var client in Kernel.GamePool.Values) Rankings(client);
                        ...
                        else if (pState == States.T_Wait)
                        {
                            if (TimeLeft == 0)
                            {
                                ...
                                if (doneMatches == ExtendedMatchArray.Length)
                                {
                                    ...
                                    SaveTop8(string.Format("Elite{0}", GroupID));
        ...
        {
            Players[uid].Entity.Teleport(1002, 439, 390);
            Players.Remove(uid);
        }
        ...
        public ushort TimeLeft
        {
            get
            {
                int value = (int)((pStamp.Millisecond - DateTime.Now.Millisecond) / 1000);
                if (value < 0) return 0;
                return (ushort)value;
            }
        }

        public void Update(GameClient client, int page = 0)
        {
            try
            {
                if (State == States.GUI_Top8Ranking)
        ...
        public void Rankings(GameClient client)
        {
