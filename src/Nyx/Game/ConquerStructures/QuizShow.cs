...
namespace Nyx.Server.Game.ConquerStructures
{
    public class QuizShow
    {
        public class QuizClient
        {
            ...
            public QuizClient(GameClient play)
            {
                UID = play.Entity.UID;
        ...
        public readonly int RightAnswerReward = 10;
        public void Start()
        {
            if (!Open)
            {
                LastQuestion = GameTime.Now;
                Open = true;
                foreach (var obj in Kernel.GamePool.Values)
        ...
        public void AddPlayer(GameClient client)
        {
            ...
            open.ExpBall2nd = 600;
            open.ExpBall3rd = 300;
            open.StartInTimeSecouds = (ushort)(TimeLimit - NewQuestionTime);
        ...
        public void QuizTimerCallback(int time)
        {
            ...
            {
                if (GameTime.Now > LastQuestion.AddSeconds(TimeLimit))
                {
                    if (NoQuestion == 1)
                        FirstQuestion = false;
                    ...
                    if (NoQuestion == QuestionCount)
                    {
                        ...
                        foreach (QuizClient quizClient in RegisteredUsers.Values)
                        {
                            ...
                            quizClient.Client.Send(quizRank.Encode());
                    ...
                    foreach (QuizClient quizClient in RegisteredUsers.Values)
                    {
                        ...
                        quizClient.Client.Send(quizQuestion.Encode());
                    }
                    ...
                    foreach (QuizClient quizClient in RegisteredUsers.Values)
                    {
                        ...
                        quizClient.Client.Send(CurrentRanks.Encode());
                    }
