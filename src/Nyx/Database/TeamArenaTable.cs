...
namespace Nyx.Server.Database
{
    public class TeamArenaTable
    {
        ...
        {
            using (var cmd = new NyxSqlCommand(MySqlCommandType.SELECT).Select("teamarena"))
        ...
        public static void SaveArenaStatistics(Network.GamePackets.TeamArenaStatistic stats, NpgsqlConnection conn)
        {
            if (stats == null) return;
            using (var cmd = new NyxSqlCommand(MySqlCommandType.UPDATE).Update("teamarena"))
        ...
        {
            using (var cmd = new NyxSqlCommand(MySqlCommandType.SELECT).Select("teamarena").Where("EntityID", client.TeamArenaStatistic.EntityID))
            using (MySqlReader rdr = new MySqlReader(cmd))
                ...
                {
                    using (var sqlCommand = new NyxSqlCommand(MySqlCommandType.INSERT).Insert("teamarena"))
                        sqlCommand
