...
namespace Nyx.Server.Database
{
    public static class InnerPowerTable
    {
        ...
        public static void Save()
        {
            ...
            foreach (var item in dictionary)
            {
                ...
                try
                {
                    ...
                    string SQL = "UPDATE inner_power SET \"Powers\" = @Powers WHERE \"UID\" = @UID";
                    ...
                    using (var conn = DataHolder.MySqlConnection)
                    {
                        ...
                        using (var cmd = new NpgsqlCommand())
                        {
                            ...
                            cmd.Parameters.Add("@UID", NpgsqlDbType.Integer).Value = (int)inner.UID;
                            cmd.ExecuteNonQuery();
                        }
                    }
                }
        ...
        public static void New(Client.GameClient client)
        {
            ...
            using (var cmd = new NyxSqlCommand(MySqlCommandType.SELECT))
            {
                cmd.Select("inner_power").Where("UID", client.Entity.UID);
                using (MySqlReader rdr = new MySqlReader(cmd))
                {
                    if (!rdr.Read())
                    {
                        ...
                        {
                            command.Insert("inner_power").Insert("UID", client.Entity.UID).Insert("Name", client.Entity.InnerPower.Name);
                            command.Execute();
                        }
                    }
                }
