...
namespace Nyx.Server.Database
{
    ...
    {
        public static AtomicCounter Counter = new AtomicCounter(1);
        public static void LoadDetainedItems(Client.GameClient client)
        {
            using (var cmd = new NyxSqlCommand(MySqlCommandType.SELECT).Select("detaineditems").Where("OwnerUID", client.Entity.UID))
        ...
        public static void DetainItem(ConquerItem item, Client.GameClient owner, Client.GameClient gainer)
        {
            ...
            NyxSqlCommand cmd2 = new NyxSqlCommand(MySqlCommandType.DELETE).Delete("detaineditems", "ItemUID", Item.ItemUID);
            cmd2.Execute();

            using (var cmd = new NyxSqlCommand(MySqlCommandType.INSERT).Insert("detaineditems"))
            ...
            using (var cmd = new NyxSqlCommand(MySqlCommandType.INSERT).Insert("claimitems"))
                cmd.Insert("ItemUID", item.UID).Insert("Date", Item.Date.Ticks)
                    .Insert("ConquerPointsCost", Item.ConquerPointsCost).Insert("OwnerUID", owner.Entity.UID)
                    .Insert("OwnerName", owner.Entity.Name).Insert("GainerUID", gainer.Entity.UID)
                    .Insert("GainerName", gainer.Entity.Name).Execute();
        ...
        {
            NyxSqlCommand cmd = new NyxSqlCommand(MySqlCommandType.DELETE).Delete("detaineditems", "ItemUID", item.ItemUID);
            cmd.Execute();
        }
