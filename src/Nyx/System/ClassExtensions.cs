...
namespace Nyx.Server
{
    public static class ClassExtensions
    {
        ...
        private static string remove(this string haystack, string needle)
        {
            ...
            if (needle.Length == 0) return haystack; // Prevent infinite loop and out-of-range
            if (index + needle.Length > haystack.Length) return haystack; // Prevent out-of-range
            return haystack.Substring(0, index) +
                haystack.Substring(index + needle.Length, haystack.Length - index - needle.Length);
        }
        public static bool Remove<T, T2>(this ConcurrentDictionary<T, T2> dict, T key) where T : notnull
        {
            T2 val;
            return dict.TryRemove(key, out val);
        }
        public static void Add<T, T2>(this ConcurrentDictionary<T, T2> dict, T key, T2 value) where T : notnull
        {
            dict[key] = value;
        }
        public static IDisposable Add<T>(this GameTimer<T> rule, T param)
        {
