...
namespace Nyx.Shared.Cryptography
{
    public class PasswordCipher
    {
        ...
        public static byte[] Encrypt(string password)
        {
            ...
            {
                uint bits1 = ToUInt32(bytes, index1 * 8) + PasswordCipher._bufSub[0];
                uint bits2 = ToUInt32(bytes, index1 * 8 + 4) + PasswordCipher._bufSub[1];
                ...
                FromUInt32(bits1, bytes, index1 * 8);
                FromUInt32(bits2, bytes, index1 * 8 + 4);
            }
            return bytes;
        }
        ...
        public static string Decrypt(byte[] passwordBytes)
        {
            ...
            {
                uint bits1 = ToUInt32(passwordBytes, index1 * 8);
                uint bits2 = ToUInt32(passwordBytes, index1 * 8 + 4);
                ...
                FromUInt32(bits1 - PasswordCipher._bufSub[0], passwordBytes, index1 * 8);
                FromUInt32(bits2 - PasswordCipher._bufSub[1], passwordBytes, index1 * 8 + 4);
            }

            int nullIndex = Array.IndexOf(passwordBytes, (byte)0);
            int length = nullIndex >= 0 ? nullIndex : passwordBytes.Length;
            return Encoding.ASCII.GetString(passwordBytes, 0, length);
        }
        ...
        private static void FromUInt32(uint value, byte[] data, int offset)
        {
